...
// ─── Envio de e-mail (via função serverless da Vercel) ────────────────────────
export async function enviarEmailAceite({ para, nomeDestinatario, grdNumero, titulo, emissor, linkAceite }) {
  const resp = await fetch("/api/enviar-email", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ para, nomeDestinatario, grdNumero, titulo, emissor, linkAceite }),
  });
  const data = await resp.json();
  if (!resp.ok) throw new Error(data?.erro || "Falha no envio de e-mail");
  return data;
}
